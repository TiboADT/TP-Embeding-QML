...
def Genererate_circle_points(n_points, center=(0.5,0.5), radius=0.5, seed=None):
    if seed is not None:
        np.random.seed(seed)
    angles = np.random.uniform(0, 2 * np.pi, n_points)
    r = radius * np.sqrt(np.random.uniform(0, 1, n_points))  # Uniform distribution in circle
...
def display_deformation(deformation_functions = [], range_x=(0, 1), range_y=(0, 1),n_points=500, seed=None,
    ...
    if point_generation == "random":
        ...
        points = Genererate_circle_points(n_points, center=((range_x[0]+range_x[1])/2,(range_y[0]+range_y[1])/2),
                                         radius=min((range_x[1]-range_x[0]),(range_y[1]-range_y[0]))/2, seed=seed)
        fig, ax, ax3d, values = plot_points_colored(points, mapping_functions=deformation_functions,title=title)
